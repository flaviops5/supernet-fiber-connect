...
interface AutomacaoChatWidgetProps {
  chatbotId?: string;
}

const AutomacaoChatWidget: React.FC<AutomacaoChatWidgetProps> = ({ 
  chatbotId = 'automacao-assistant' 
}) => {
        ...
        <div className="fixed bottom-28 right-6 z-50 w-[380px] h-[600px] max-w-[calc(100vw-3rem)] max-h-[calc(100vh-8rem)] bg-card rounded-2xl shadow-2xl border-2 border-primary/20 overflow-hidden animate-scale-in">
          ...
          <div className="bg-gradient-hero text-white p-4 flex items-center gap-3">
            ...
            <div className="flex-1">
              <h3 className="font-semibold">Assistente de Automação</h3>
          ...
          <div className="h-[calc(100%-4rem)] bg-background">
            {isLoaded && (
              <iframe
                ref={iframeRef}
                src={`https://mxdupkbpxjcfxdgrwknp.supabase.co/functions/v1/automacao-agent?chatbot_id=${chatbotId}`}
                className="w-full h-full border-0"
                title="Chat de Automação Residencial"
                allow="microphone"
              />
            )}
            {!isLoaded && (
        ...
        <div className="fixed bottom-28 right-6 z-40 animate-fade-in">
          <div className="bg-white rounded-lg shadow-lg p-3 max-w-xs border border-border">
            <p className="text-sm text-foreground">
              <strong>Dúvidas sobre automação?</strong><br />
              Fale com nosso assistente especializado!
            </p>
          </div>
        </div>
      )}
...
export default AutomacaoChatWidget;

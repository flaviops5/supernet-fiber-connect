...
const HeroSection = () => {
  ...
  const handleWhatsApp = () => {
    const message = 'Olá! Quero conhecer os planos de internet fibra da SUPERNET FIBRA.';
    window.open(`https://wa.me/5511999999999?text=${encodeURIComponent(message)}`, '_blank');
  };

  return (
    <section id="sobre" className="relative bg-gradient-subtle min-h-[80vh] flex items-center overflow-hidden">
                  ...
                  {slides.map((_, index) => (
                    <button 
                      ...
                      className={`h-1.5 rounded-full transition-all ${
                        index === currentSlide ? 'bg-white w-1.5' : 'bg-white/30 hover:bg-white/50 w-1.5'
                      }`}

...
Deno.serve(createAuthenticatedHandler(
  ...
  async (req, { supabase, user }) => {
    ...
    console.log('Primeiros 5 IDs:', registros.slice(0, 5).map((r: IXCPlanRecord) => ({ id: r.id, tipo: typeof r.id, nome: r.grupo })));
    console.log('IDs buscados:', planIds);

    // Filtrar pelos IDs solicitados - tentar múltiplos formatos
    const plansToSync = registros.filter((r: IXCPlanRecord) => {
      const rid = r.id;
    ...
    if (plansToSync.length === 0) {
      ...
      const allIds = registros.slice(0, 20).map((r: IXCPlanRecord) => r.id);
